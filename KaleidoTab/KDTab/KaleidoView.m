...
-(void) drawToLayer
{
   ...
   // printf("%f",self.view_width);printf("\n");
    ...
    [self setSh:(int) shape];
    printf("shaplevallallaal %d",shape);
...
- (void) updateTimer {
    ...
    if (timer) {
        [timer invalidate];
       // printf("timer--%2lf",timer);
        timer = nil;
    }
    
    // set up new timer with new frame rate
    float delay = 1.0/frameRate;
    printf("timer %f",frameRate);
    printf("\n");
...
-(void) drawRectUsing:(CGRect) cgrect withColor:(UIColor *)color
{
    ...
    if(shape==1){
        ...
        printf("shape rect%d",shape);
        printf("\n");
        
        
        CGContextFillRect(layer_context, cgrect);
        CGContextAddRect(layer_context, cgrect);
    }
    else if(shape==0){
        
        //CGContextFillEllipseInRect(layer_context, cgrect);
        printf("shape eclipse%d",shape);
       CGContextAddRect(layer_context, cgrect);
        //CGContextAddArc(layer_context, self.rect_x, self.rect_y, 20, 0, 2*M_PI, 0);
    }
    
    //colorfill
    if(val==1){
      //CGContextDrawPath(layer_context,kCGPathStroke);
   CGContextDrawPath(layer_context, kCGPathStroke);
    }
    else if (val==0){
